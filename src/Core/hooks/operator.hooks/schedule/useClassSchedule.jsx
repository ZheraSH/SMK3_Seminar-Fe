...
export default function useClassSchedule(classroomId, activeDayApi) {
    ...
    const saveSchedule = useCallback(async (data, isEditMode) => {
        ...
        try {
            ...
            console.log(`Jadwal Berhasil ${isEditMode ? 'Diperbarui' : 'Dibuat'}:`, result);
            return result;
            
        } catch (err) {
            console.error(`Gagal ${isEditMode ? 'memperbarui' : 'menambahkan'} jadwal:`, err.response?.data || err);
    ...
    const removeSchedule = useCallback(async (scheduleId) => {
        setError(null);
        if (!scheduleId) {
            setError({ message: "ID Jadwal diperlukan untuk penghapusan.", type: 'client' });
    ...
    return { classSchedule, isLoadingSchedule, error, refetch, saveSchedule, removeSchedule, };
}