...
import { Search, ChevronDown, ChevronRight, Edit3, Trash2, Eye } from "lucide-react"
...
export const TeacherMainyuyu = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        await axios.post(`http://127.0.0.1:8000/api/employees/${editingId}?_method=PUT`, formData, {
  ...
  const validateForm = () => {
    ...
    if (!post.phone_number) newErrors.phone_number = ["Nomer telepon wajib diisi."]
    if (!post.address) newErrors.address = ["Alamat wajib diisi."]
    if (!post.religion_id) newErrors.religion_id = ["Agama wajib dipilih."]
    if (!post.roles || post.roles.length === 0) newErrors.roles = ["Role wajib dipilih."]
    ...
    if (currentFilter.type && currentFilter.value) {
      switch (currentFilter.type) {
        case "gender":
          ...
          break
        case "religion":
          matchesFilter = teacher.religion?.name?.toLowerCase() === currentFilter.value.toLowerCase()
          break
        default:
          matchesFilter = true
      }
    ...
    <div className="p-6 ">
      ...
      <div className="flex justify-between items-center mb-5 gap-5">
        ...
        <div className="relative" ref={categoryRef}>
          ...
          >
            <span className={`${category === "Pilih Kategori" ? "text-black" : "text-black"}`}>{category}</span>
            <ChevronDown
          ...
          {openCategory && (
            <div className="absolute top-full mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-200 p-2 z-20">
              ...
              <div className="border-t border-gray-100">
                <button
                  onClick={() => setOpenSubMenu(openSubMenu === "religion" ? "" : "religion")}
                  className="flex justify-between items-center w-full text-left text-sm px-3 py-2 hover:bg-gray-50 rounded-lg text-gray-700 transition"
                >
                  Agama
                  <ChevronRight
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-[700px] px-10 py-8 relative">
              ...
              <div className="flex items-start justify-between mb-6">
                <h2 className="text-lg font-semibold text-gray-800">Detail Guru</h2>
              </div>
              ...
              <div className="grid grid-cols-2 text-gray-700 text-[15px] gap-y-2 mb-4">
                <div className="space-y-7">
                  ...
                  <p>
                    <span className="font-medium">NIK :</span> {selectedTeacher.NIK || "-"}
                  </p>
                  <p>
                    <span className="font-medium">Agama :</span> {selectedTeacher.religion?.name || "-"}
                  </p>
                  <p>
                    <span className="font-medium">Tanggal Lahir :</span> {selectedTeacher.birth_date || "-"}
                  </p>
                  <p>
                    <span className="font-medium">No Telepon :</span> {selectedTeacher.phone_number || "-"}
                  </p>
                </div>

                <div className="space-y-7">
                  <p>
                    <span className="font-medium">NIP :</span> {selectedTeacher.NIP || "-"}
                  </p>
                  <p>
                  ...
                  <p>
                    <span className="font-medium">Tempat Lahir :</span> {selectedTeacher.birth_place || "-"}
                  </p>
                  <p>
                    <span className="font-medium">Role :</span>{" "}
                    {Array.isArray(selectedTeacher.roles)
                      ? selectedTeacher.roles.map((r) => r.label || r.value || r.name || r).join(", ")
                      : "-"}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 text-gray-700 text-[15px]">
                <p>
                  <span className="font-medium">Alamat :</span> {selectedTeacher.address || "-"}
                </p>
              </div>
        ...
        {isOpen && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl w-[700px] max-h-[90vh] overflow-y-auto p-6 relative">
              ...
              <h2 className="text-2xl font-semibold mb-6 text-gray-800">Tambah Guru</h2>
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInput}
                  />
                  {errors.NIK && <p className="text-red-500 text-sm mt-1">{errors.NIK[0]}</p>}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInput}
                  />
                  {errors.NIP && <p className="text-red-500 text-sm mt-1">{errors.NIP[0]}</p>}
                </div>

                {/* Field Tempat Lahir */}
                <div>
                  <label className="block text-sm font-medium text-gray-600">Tempat Lahir *</label>
                  <input
                    ...
                    onChange={handleInput}
                  />
                  {errors.birth_place && <p className="text-red-500 text-sm mt-1">{errors.birth_place[0]}</p>}
                </div>
                ...
                {/* Field Agama */}
                <div>
                  <label className="block text-sm font-medium text-gray-600">Agama *</label>
                  <select
                    name="religion_id"
                    className={`border rounded-lg p-2 w-full focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  ...
                  {errors.religion_id && <p className="text-red-500 text-sm mt-1">{errors.religion_id[0]}</p>}
                </div>

                {/* Field Nomer Telfon */}
                <div>
                  <label className="block text-sm font-medium text-gray-600">Nomer Telfon *</label>
                  <input
                    className={`border rounded-lg p-2 w-full ${
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleInput}
                    value={Array.isArray(post.roles) && post.roles.length > 0 ? post.roles[0]?.value || "" : ""}
                  >
                    <option value="">Pilih Role</option>
                    <option value="teacher">Guru Pengajar</option>
                    <option value="homeroom_teacher">Wali Kelas</option>
                    <option value="curriculum_coordinator">Waka Kurikulum</option>
                  </select>
      ...
      <table className="min-w-full border border-gray-200 rounded-lg overflow-hidden shadow-sm">
        <thead>
          <tr className="bg-[#3B82F6] text-white text-sm">
            ...
            <th className="px-4 py-3 text-left font-semibold border-r border-[#3B82F6]">NIK</th>
            <th className="px-4 py-3 text-left font-semibold border-r border-[#3B82F6]">NIP</th>
            <th className="px-4 py-3 text-left font-semibold border-r border-[#3B82F6]">Email</th>
            <th className="px-4 py-3 text-left font-semibold border-r border-[#3B82F6]">No Telepon</th>
          ...
          {currentTeachers.length > 0 ? (
            currentTeachers.map((teacher, index) => (
              <tr key={teacher.id} className="border-t border-gray-200 hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-3">{teacher.NIK}</td>
                ...
                <td className="px-4 py-3 text-center">
                  <div className="relative inline-block text-left">
                    <button
                      onClick={() => setOpenItemId(openItemId === teacher.id ? null : teacher.id)}
