...
const ScheduleDetailPage = ({ selectedClassroomData, handleBackToClasses }) => {
    ...
    const handleDeleteSchedule = (itemData) => {
        setOpenDropdownId('');
        setConfirmModal({ show: true, item: itemData, message: `Yakin ingin menghapus mata pelajaran ${itemData.subject} jika anda belum yakin batalkan.`});
    };
    ...
    <div>
        ...
        <div className="flex flex-col md:flex-row md:justify-between items-center pb-2 mb-4 bg-white rounded-lg drop-shadow-sm p-2 md:h-[60px] gap-3 md:gap-0">
            <div className="flex space-x-1 bg-[#EFF1F3] w-full md:w-auto overflow-x-auto p-1 rounded-lg flex-shrink-0">
                {days.map(day => (
                    <button key={day} onClick={() => setActiveDay(day)} className={`flex-shrink-0 px-4 py-2 text-sm font-medium rounded-lg transition duration-200 
            ...
            <div className=" rounded-md overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 border border-gray-200 ">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                        {isLoadingSchedule && (
                            <tr>
                                <td colSpan={6} className="px-4 py-8 text-center text-blue-500 font-semibold text-center"> Memuat jadwal hari {activeDay}...</td>
                            </tr>
                        )}
                        {!isLoadingSchedule && schedulesToDisplay.length > 0 ? (
                                ...
                                return (
                                    <tr key={item.id || item.lesson_hour_id} className={'hover:bg-gray-50'}> 
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-center relative">
                                            {item.id ? ( 
                                                <>
                                                    <button onClick={() => toggleDropdown(item.id)} className='p-1 text-gray-500 hover:text-gray-700 focus:outline-none'><MoreVertical className='w-5 h-5'/></button>
                                                    {openDropdownId === item.id && (
                                                        <div ref={dropdownRef} className="absolute top-0 right-0 transform -translate-x-1/2 w-40 bg-white border border-gray-200 rounded-md shadow-xl z-20">
                                                            <button  onClick={() => openEditModal(item)} className="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                                                <PencilLine className="w-4 h-4 mr-3 text-[#FACC15]" />Edit
                                                            </button>
                                                            <button onClick={() => handleDeleteSchedule(item)} className="flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50">
                            ...
                            <tr>
                                <td colSpan={6} className="px-4 py-8 text-center text-gray-500"> Tidak ada jadwal pelajaran untuk hari {activeDay} ini. </td>
                            </tr>
                        )}
                        <tr>
                            <td colSpan={6} className='text-center py-2' >
                                <button onClick={openAddModal} className="inline-flex items-center space-x-2 bg-[#3B82F6] text-white py-2 px-4 rounded-lg font-medium text-[14px] transition duration-200 shadow-sm hover:bg-blue-700">
                                    <Plus size={16} /> <span>Tambah Mata Pelajaran</span>
                                </button>
                            </td>
                        </tr>
                    </tbody>
        ...
        <DeleteConfirmModal open={confirmModal.show} message={confirmModal.message} onConfirm={executeRemoval}  onCancel={() => setConfirmModal({ show: false, item: null, message: '' })} />
    </div>
    );
};
