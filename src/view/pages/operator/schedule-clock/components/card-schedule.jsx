...
const CardList = ({ schedule, handleViewSchedule}) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
        {schedule.map((data, index) => (
            <div key={index} className="bg-white h-[172px] rounded-lg p-4 drop-shadow-md hover:shadow-xl transition duration-300 border border-gray-100 cursor-pointer">
                ...
                <div className='space-y-2 mb-1 flex justify-between items-center'>
                  <div className="text-gray-600">
                    ...
                    <p className="font-medium text-[12px] text-gray-700">{data.homeroom_teacher}</p>
                  </div>
                  <div className="flex items-center text-gray-600">
                    <span className='mr-1 text-lg'><User size={20} /></span>
                    <span className='font-medium text-[#444444] text-[12px]'> {data.total_students}</span>
                  </div>
                </div>
