                            ...
                            schedules.map((item, index) => (
                                <tr key={item.id || item.lesson_hour_id} className={`${index % 2 === 1 ? 'bg-[#EFF6FF]' : 'bg-white'}`}>
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap text-[14px]">
                                        <span className={`inline-block px-3 py-1 rounded-full text-white text-[12px] font-semibold shadow-sm
                                            ${item.lesson_hour?.name?.toLowerCase().includes('istirahat') ? 'bg-[#F59E0B]' : 'bg-[#22C55E]'}`}>
                                            {item.lesson_hour?.time || '—'}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3 whitespace-nowrap text-[14px] text-gray-700 font-sm">
                                        {item.subject ? (
                                            <span className={`inline-block px-3 py-1 rounded-full text-white text-[12px] font-medium shadow-sm ${getBgColorBySubject(item.subject.name)}`}>
                                                {item.subject.name}
                                            </span>
