...
import useMajors from '../../../../Core/hooks/class-major/useMajor'; 
                    ...
                    {renderedMajors.map((major) => (
                        <div key={major.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-6 flex flex-col items-center text-center border border-gray-100 ">
                            ...
                            <p className="text-base md:text-md font-semibold text-gray-800 h-10 flex flex-col justify-center items-center mb-4">
                                {major.fullName} 
                                {major.apiName && <span className="text-sm font-normal text-gray-600">({major.apiName})</span>}
                            </p>     
                            <button onClick={() => handleViewDetail(major.fullName)}className="w-full py-2 bg-[#3B82F6] text-white font-medium rounded-lg transition-colors duration-300 shadow-md shadow-blue-300/50">Lihat Detail</button> 
                        </div>
                    ))}
                </div>
                )}
