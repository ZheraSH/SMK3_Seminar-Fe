...
function Dropdown({ label, value, placeholder, data = [], onChange, error }) {
 ...
 return (
  <div className="w-full relative">
    <label className="block text-gray-700 text-sm font-bold ml-1">{label}</label>
    <div onClick={() => setOpen(!open)} className={`w-full px-4 py-3 bg-gray-200 border ${  error ? "border-red-500" : "border-gray-300"} rounded-xl flex justify-between items-center cursor-pointer`}>
...
export default function Form({ onClassAdded, addClass, onError }) {
  ...
  const activeSchoolYears = schoolYears.filter(year => year.active === true);
 ...
 return (
  <form onSubmit={handleSubmit}>
    <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-6">
      <Dropdown label="Tingkatan" placeholder="Pilih Tingkatan" data={levelClass || []} value={levelClassId} 
        onChange={(val) => { setLevelClassId(val); setErrors({ ...errors, levelClassId: "" });}} error={errors.levelClassId}/>

      <Dropdown label="Jurusan" placeholder="Pilih Jurusan" data={majors || []} value={majorId}
      onChange={(val) => { setMajorId(val); setErrors({ ...errors, majorId: "" });}}  error={errors.majorId} />

      <div className="w-full">
        <label className="block text-gray-700 text-sm font-bold ml-1"> No Kelas</label>
      <input type="number" placeholder="Masukkan No" className={`w-full px-4 py-3 bg-gray-200 border ${ errors.classNameInput ? "border-red-500" : "border-gray-300"} rounded-xl`} value={classNameInput}
        onChange={(e) => { setClassNameInput(e.target.value); setErrors({ ...errors, classNameInput: "" });}}/>
      ...
      <Dropdown label="Tahun Ajaran" placeholder="Pilih Tahun Ajaran" data={activeSchoolYears} value={schoolYearId}
        onChange={(val) => { setSchoolYearId(val); setErrors({ ...errors, schoolYearId: "" }); }} error={errors.schoolYearId}/>

      <Dropdown label="Wali Kelas" placeholder="Pilih Guru Pengajar" data={teachers || []} value={teacherId}
        onChange={(val) => { setTeacherId(val); setErrors({ ...errors, teacherId: "" }); }} error={errors.teacherId} />
      </div>
    </div>
