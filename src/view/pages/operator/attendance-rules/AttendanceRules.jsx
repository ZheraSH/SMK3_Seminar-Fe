...
export default function AttendanceRulesPage() {
  ...
  if (loading && attendanceRules.length === 0) return <p className="text-center mt-10">Memuat data...</p>;
  ...
  return (
    <div className="mx-3 sm:mx-5">
      ...
      <div className="bg-white mt-8 w-full rounded-lg lg:h-auto p-4 mb-6 drop-shadow-md">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar rounded-lg p-2 mb-2 w-full">
          {attendanceRules.map((rule) => (
            <button key={rule.day} onClick={() => { setSelectedDay(rule.day); setError(null); setSuccess(null); setFieldErrors({});}}
              className={`px-5 py-2 rounded-lg text-sm whitespace-nowrap transition-all drop-shadow-sm
                ${ selectedDay === rule.day ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200 border border-[#CBD5E1]"}`}>
              {rule.day_label}
            </button>
          ))}
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="bg-white border border-gray-200 p-4 sm:p-6 rounded-xl shadow-sm">
                <div className="flex items-center mb-4"> <span className="text-[18px] font-semibold">Jam Masuk</span> </div>
                <div className="flex flex-col lg:flex-row lg:items-center lg:gap-6 gap-4">
                  <div className="flex-1">
                    ...
                    <input type="time" value={selectedDayData.checkin_start?.slice(0, 5) || ""} onChange={(e) => handleTimeChange("checkin_start", e.target.value)} className={`w-full px-3 py-2 border rounded-md outline-none transition-all 
                  ...
                  <div className="flex-1">
                    ...
                    <input type="time" value={selectedDayData.checkin_end?.slice(0, 5) || ""} onChange={(e) => handleTimeChange("checkin_end", e.target.value)} className={`w-full px-3 py-2 border rounded-md outline-none transition-all 
                      ${ fieldErrors.checkin_end ? "border-red-500 bg-red-50" : "border-gray-300 focus:border-blue-500"}`}/>
                      {fieldErrors.checkin_end && <p className="text-red-500 text-xs mt-1">{fieldErrors.checkin_end}</p>}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 p-4 sm:p-6 rounded-xl shadow-sm">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:gap-6 gap-4">
                  <div className="flex-1">
                    <p className="mb-2 text-sm font-medium text-gray-600">Mulai</p>
                    <input type="time" value={selectedDayData.checkout_start?.slice(0, 5) || ""} onChange={(e) => handleTimeChange("checkout_start", e.target.value)} className={`w-full px-3 py-2 border rounded-md outline-none transition-all 
                  ...
                  <div className="flex-1">
                    <p className="mb-2 text-sm font-medium text-gray-600">Selesai</p>
                    <input type="time" value={selectedDayData.checkout_end?.slice(0, 5) || ""} onChange={(e) => handleTimeChange("checkout_end", e.target.value)} className={`w-full px-3 py-2 border rounded-md outline-none transition-all 
                      ${fieldErrors.checkout_end ? "border-red-500 bg-red-50" : "border-gray-300 focus:border-blue-500"}`}/>
                      {fieldErrors.checkout_end && <p className="text-red-500 text-xs mt-1">{fieldErrors.checkout_end}</p>}
                  </div>
                </div>
              </div>
            </div>

            <div className="flex flex-row justify-between items-center gap-4 mt-8 pt-6">
              <label htmlFor="holiday-toggle" className="flex items-center gap-4 cursor-pointer group">
                <input  id="holiday-toggle"  type="checkbox"  checked={selectedDayData.is_holiday}  onChange={handleHolidayChange}  className="sr-only peer"/>
              ...
              <button onClick={handleSave} disabled={saving || loading} className={`flex items-center justify-center gap-2 px-8 py-3 rounded-lg font-medium transition-all
               ${ saving || loading ? "bg-gray-400 cursor-not-allowed text-white" : "bg-blue-600 hover:bg-blue-700 text-white shadow-md active:scale-95" }`} >
