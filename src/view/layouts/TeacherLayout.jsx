  ...
  return (
    <>
      ...
      <div className="flex h-screen bg-gray-50">
        <div className={`fixed top-0 left-0 h-full z-40 transform transition-all duration-300 ${
          sidebarOpen ? "w-[250px] translate-x-0" : "w-[80px] -translate-x-full lg:translate-x-0"}`}>
          <div className={`h-full bg-white border-r border-gray-200 ${sidebarOpen ? "" : "overflow-hidden"}`}>
            <div className={`flex justify-center items-center gap-3 py-6 ${sidebarOpen ? "px-10" : ""}`}>
              ...
              {sidebarOpen && (
                <h1 className="flex-wrap text-[16px] font-bold text-gray-800"> 
            ...
            <div ref={scrollRef} className={`overflow-y-auto ${ 
              sidebarOpen ? "h-[calc(100vh-100px)] [&::-webkit-scrollbar]:hidden" : "h-[calc(100vh-80px)]"}`}>
              <div className="flex flex-col gap-1">
                {menuItemTeacher.map((item, index) => {
                  ...
                  return (
                    <div key={index}>
                      <Link  to={item.path}  className={`relative flex items-center mb-1.5 hover:bg-[#E5F0FF] transition-colors ${
                          sidebarOpen ? "gap-3 p-2 pl-5" : "justify-center p-3"
                        } ${active ? "bg-[#3B82F61F] text-[#3B82F6]" : "text-gray-600"}`} 
              ...
              {showScrollButton && sidebarOpen && (
                <button onClick={scrollToBottom} className="absolute bottom-4 right-4 bg-white border border-gray-100 rounded-full p-2 shadow-lg hover:bg-gray-50">
