...
export const DashboardLayouth = (props) => {
  ...
  return (
    <>
      <div className="flex">
      <div className="bg-[#1E3A8A] w-[270px] h-screen fixed">
      ...
      <div className="p-6 text-white">
        {menuItems.map((item, index) => (
          <Link
            key={index}
            to={item.path}
            className={`flex items-center gap-3 p-3.5 text-[16px] font-bold mb-3 cursor-pointer duration-300 hover:bg-white hover:rounded-full hover:text-[#1E3A8A] ${
        ...
        <div className="ml-[270px] flex-1 h-screen overflow-y-auto bg-gray-50">
          <MainDashboard/>
          <Outlet/>
        </div>
      </div>
